...
<div class="page-hero">
    ...
    <div class="page-hero-content">
        <h1 class="page-hero-title" data-aos="fade-up">{{ $blog->title }}</h1>
        <nav aria-label="breadcrumb" data-aos="fade-up" data-aos-delay="120">
            <ol class="breadcrumb justify-content-center mb-0">
                ...
                <li class="breadcrumb-item"><a href="{{ route('blogs.get') }}">Blogs</a></li>
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-8" data-aos="fade-up">
                <div class="bg-white rounded-3 shadow-sm overflow-hidden">
                    <img src="{{ Storage::url($blog->blog_image_path) }}"
                    ...
                    <div class="p-4 p-md-5">
                        <div class="d-flex flex-wrap gap-3 align-items-center mb-4 pb-3 border-bottom">
                            <span class="text-muted small">
                                <i class="fa-regular fa-user text-danger me-1"></i>{{ $blog->author }}
                            </span>
                            <span class="text-muted small">
                                <i class="fa-regular fa-calendar-days text-danger me-1"></i>
                                {{ $blog->published_date->format('F j, Y') }}
                            </span>
                        </div>
                    ...
                    <ul class="list-unstyled mb-0">
                        @foreach ($blogs as $latestBlog)
                            <li class="d-flex gap-3 mb-3 pb-3 border-bottom">
                                <img src="{{ Storage::url($latestBlog->blog_image_path) }}"
                                     class="rounded flex-shrink-0"
                                     style="width:64px; height:64px; object-fit:cover;"
                                     alt="{{ $latestBlog->title }}">
                                <div>
                                    <a href="{{ route('blog.detail.get', $latestBlog->slug) }}"
                                       class="text-decoration-none fw-semibold sidebar-blog-titles-link"
                                    ...
                                    <p class="text-muted mb-0 mt-1" style="font-size:12px;">
                                        <i class="fa-regular fa-calendar-days text-danger me-1"></i>
                                        {{ $latestBlog->published_date->format('M j, Y') }}
                                    </p>
                                </div>
                            </li>
                        @endforeach
